...
class FSMState{
    ...
    private:
        FSMAction action;
        std::vector<std::pair<T,FSMState&>> transitions;
    public:
        int id;
        ...
        FSMState(int id, FSMAction action, std::vector<std::pair<T,FSMState&>> transitions){
        ...
        void AddTransition(T event, FSMState<T>& to_state){
            this->transitions.push_back(std::pair<T,FSMState&>(event,to_state));
        };

        std::pair<T,FSMState&> GetTransitionByEvent(T event){
            for (auto transition : this->transitions){
                if (transition.first == event){
                    return transition;
                }
        }
    };
        ...
        int** GenerateMatrix(){
            ...
            for(size_t i = 0; i != this->states.size(); i++){
                ...
                for(size_t j = 0; j != this->events.size(); j++){
                    ...
                    array[i][j] = transition.second.id;
                }
            }
            return array;
        };

        static void Merge(FSM* fsm_1, FSM* fsm_2){
            // DO MERGE 
        };
